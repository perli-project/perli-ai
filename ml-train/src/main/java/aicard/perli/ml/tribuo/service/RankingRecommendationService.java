...
public class RankingRecommendationService {
    ...
    private final RankingInferenceService inferenceService;

    /**
     * @param modelPath 학습 완료된 .gdpc 모델 파일의 경로
     */
    public RankingRecommendationService(String modelPath) {
        this.inferenceService = new RankingInferenceService(modelPath);
        log.info("랭킹 추천 엔진이 모델을 로드하여 가동되었습니다: {}", modelPath);
    }
     ...
     */
    public List<CardResponse> getRankedRecommendations(List<CardRequest> candidates) {
     ...
     */
    public List<CardResponse> getTopNRecommendations(List<CardRequest> candidates, int n) {
