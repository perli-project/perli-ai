## ✨ AI 주요 기능
> AI 스마트 카드 실적 매니저의 AI 엔진은 마이데이터로 수집된 카드 사용 내역을 분석하여 <br>
사용자별 소비 패턴을 학습하고, 전월 실적 달성 가능성 예측,<br>
카드별 혜택 최적화 전략, 지출 분배 추천을 자동 생성하는 지능형 모델입니다.<br>
규칙 기반과 머신러닝 기반을 결합한 하이브리드 AI 구조로 높은 정확성과 개인화된 추천을 제공합니다.
<br>

## ⚙️ 기술 스택

<table>
  <tr>
    <th>Category</th>
    <th>Stack</th>
  </tr>
  <tr>
    <td><strong>소비 패턴 분석</strong></td>
    <td>
      <img src="https://img.shields.io/badge/Tribuo-E35A5A?style=for-the-badge">
      <img src="https://img.shields.io/badge/deep learning4j-E35A5A?style=for-the-badge">
<img src="https://img.shields.io/badge/nd4j-E35A5A?style=for-the-badge">
    </td>
    
  </tr>
  <tr>
    <td><strong>카드 순위화 및 추천</strong></td>
    <td>
      <img src="https://img.shields.io/badge/H2o.ai-FF8A2A?style=for-the-badge">
    </td>
  </tr>
  <tr>
    <td><strong>카드 실적 최적 플랜</strong></td>
    <td>
        <img src="https://img.shields.io/badge/TimeFold-FFCC00?style=for-the-badge">
    </td>
  </tr>
</table>

<br>

## 🤖 AI 시스템 아키텍처


<br>

## 🤝 협업 규칙
우리 프로젝트는 원활한 협업을 위해 아래와 같은 규칙을 따릅니다.

- **[Commit Convention](./.github/COMMIT_CONVENTION.md)**

<br>

## 📊 AI 참고자료 출처


<br>

## 💁‍♂️ 팀원 소개

<table align="center">
  <tr>
    <td align="center">
      <a href="https://github.com/WhiteBin-bin">
        <img src="https://github.com/WhiteBin-bin.png" alt="백현빈 프로필" width="150" height="150"/><br>
        <b>백현빈</b>
      </a>
    </td>
    <td align="center">
      <a href="https://github.com/saranghein">
        <img src="https://github.com/saranghein.png" alt="이해인 프로필" width="150" height="150"/><br>
        <b>이해인</b>
      </a>
    </td>
  </tr>
</table>
